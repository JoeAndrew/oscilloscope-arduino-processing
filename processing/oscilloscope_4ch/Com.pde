class Com{
   ...
   void display(){
     ...
     fill(cor); rect(x,y-20,10*u,20);
     rect(x,y,u,h); rect(x+u,y,6*u,h); rect(x+7*u,y,2*u,h); rect(x+9*u,y,u,h);  
     fill(0);textAlign(CENTER,CENTER); text("Configure Serial port",x+w/2,y-12);
     text("*",x+u/2,y+h/2); text(ports[indPort],x+4*u,y+h/2); text(speeds[indSpeed],x+8*u,y+h/2); 
     if (conectado) tex="on"; else tex="off";
     text(tex,x+9.5*u,y+h/2);//9*u,y+h/2);
   }
